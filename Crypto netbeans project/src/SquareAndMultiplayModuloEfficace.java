...
public class SquareAndMultiplayModuloEfficace extends javax.swing.JFrame {

    /**
     * Creates new form mainJfram
     */
    public SquareAndMultiplayModuloEfficace() {
        initComponents();
...
/////////////////////////////////////////////////////////////////////////
	 ...
	 class modPowSM_Worker extends SwingWorker< BigInteger ,Void > {
  
             BigInteger b; //base
             BigInteger p; //power
             BigInteger m; //modulo
    public modPowSM_Worker (BigInteger B, BigInteger P, BigInteger M ) 
    {
     b=B;
     p=P;
     m=M;
    ...
    @Override
       public BigInteger doInBackground() throws InterruptedException, IOException { // type 1
           ...
           String bBinaryStr = p.toString(2); // 2 pour dire en base 2 (en binaire)
      BigInteger aPOWb= BigInteger.valueOf(1); //aPOWb = 1//aPOWb = 1
        
      for(int i=0; i<bBinaryStr.length();i++)   // pour i parcourant toute la chaine de la représentation binaire de b
      { 
          if(bBinaryStr.charAt(i)=='1') // si le bit est 1 alors SM
          {
             /* publish("Byte ("+String.valueOf(i+1)+") = 1   (SM):\n >>>>>>>>>>>>>>>>>>>>>>>>\n");*/
            //  fw.write("Byte ("+String.valueOf(i+1)+") = 1   (SM):\n >>>>>>>>>>>>>>>>>>>>>>>>\n");
              
        /* publish( "S:  ( "+aPOWb.toString()+")"+"^2 = ");*/
        // fw.write("S:  ( "+aPOWb.toString()+")"+"^2 = ");
              aPOWb= aPOWb.multiply(aPOWb);  // squar
              aPOWb=aPOWb.mod(m);  // reduction
        /*  publish(aPOWb.toString()+"\n");*/
        //  fw.write(aPOWb.toString()+"\n");
          
         /*  publish( "M:  ( "+aPOWb.toString()+") * "+b.toString()+" = ");*/
         //  fw.write("M:  ( "+aPOWb.toString()+") * "+b.toString()+" = ");
           aPOWb= aPOWb.multiply(b); // multiply
           aPOWb=aPOWb.mod(m);  // reduction
         /* publish(aPOWb.toString()+"\n\n");*/
         // fw.write(aPOWb.toString()+"\n\n");
          }
          else{  
             /* publish("Byte ("+String.valueOf(i+1)+") = 0    (S) :\n >>>>>>>>>>>>>>>>>>>>>>>>\n ");*/
             // fw.write("Byte ("+String.valueOf(i+1)+") = 0    (S) :\n >>>>>>>>>>>>>>>>>>>>>>>>\n ");
             /* publish( "S:  ( "+aPOWb.toString()+")"+"^2 = ");*/
              //fw.write( "S:  ( "+aPOWb.toString()+")"+"^2 = ");
              aPOWb= aPOWb.multiply(aPOWb);
              aPOWb=aPOWb.mod(m); 
         /* publish(aPOWb.toString()+"\n\n");*/
         // fw.write(aPOWb.toString()+"\n\n");
              }  //sinon 0 et donc squar
      }
      /* publish(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n"
               + "Calcule finis: ( "+b.toString()+") ^ ( "+p.toString()+" = "+aPOWb.toString()+
               "\n<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n"
               );*/
      /* fw.write(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n"
               + "Calcule finis: ( "+b.toString()+") ^ ( "+p.toString()+" = "+aPOWb.toString()+
       ...
       protected void done() {
            ...
            onWork=false;
                 ...
                 try {   
                     ...
                     else{ jTextArea2.setText("Le nombre est trés grand pour l'afficher ici!! Ouvrer le dans un fichier txt! cliquer en bas");
...
/////////////////////////////////////////////////////////////////////////////////////////////
      ...
      @Override
       public void done() { // once the thread finish this function start and in the events dispatch thread
         try {
        jTextArea2.append(" ( il contient "+get().toString()+ " chiffres dicimaux ). Il est de "+bitsNum+" bits");
  ...
  protected void process(final List<String> chunks) { // hee type2
   jTextArea1.setText("");
    ...
    private void initComponents() {
        ...
        jLabel6.setText("   Aller à Squar and multiply ( Educative )");
        ...
        jLabel18.setText(" square and multiply (modulaire pratique)");
        ...
        jTextField1.setBounds(150, 70, 100, 20);

        jLabel19.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        jLabel19.setForeground(new java.awt.Color(128, 86, 159));
        jLabel19.setText("mod");
        jPanel11.add(jLabel19);
        jLabel19.setBounds(230, 110, 40, 17);

        jTextField3.setBackground(new java.awt.Color(40, 36, 52));
        jTextField3.setForeground(new java.awt.Color(89, 85, 113));
        jTextField3.setText("   Base");
        ...
        jTextField3.setBounds(53, 104, 140, 20);
        ...
        jLabel26.setText("le fichier généré");
        jPanel21.add(jLabel26);
        jLabel26.setBounds(314, 107, 94, 17);
        ...
        jLabel27.setBounds(170, 90, 10, 17);

        jTextField4.setBackground(new java.awt.Color(40, 36, 52));
        jTextField4.setForeground(new java.awt.Color(89, 85, 113));
        jTextField4.setText("  Modulo");
        jTextField4.setBorder(javax.swing.BorderFactory.createMatteBorder(1, 1, 1, 1, new java.awt.Color(41, 54, 66)));
        ...
        jTextField4.setBounds(280, 108, 80, 20);
...
modPowSM_Worker workerPow; // my worker reference
...
if(!onWork){
 ...
 jLabel23.setVisible(false);
                      jLabel25.setVisible(false);
                       jLabel26.setVisible(false);
                        jLabel28.setVisible(false);
        if(!(jTextField1.getText().equals("  Puissance")||jTextField3.getText().equals("   Base")
...
workerPow = new modPowSM_Worker(new BigInteger(jTextField3.getText()), new BigInteger(jTextField1.getText()),new BigInteger(jTextField4.getText()));
workerPow.execute();
...
if(jTextField3.getText().equals("   Base"))
jTextField3.setText("");
...
jTextField4.setBackground(new java.awt.Color(51, 46, 66));  
jTextField4.setForeground(new java.awt.Color(129, 126, 150));  
if(jTextField4.getText().equals("  Modulo"))
...
jTextField4.setForeground(new java.awt.Color(89, 85, 113));
 if(jTextField3.getText().equals(""))
jTextField4.setText("  Modulo");        // TODO add your handling code here:
...
jTextField4.setFocusable(true);
        if(!jTextField3.isFocusOwner()){
       jTextField4.setBackground(new java.awt.Color(46, 41, 60)); 
jTextField4.setForeground(new java.awt.Color(105, 101, 130)); }
// TODO add your handling code here:
...
if(length>1){
        char c =jTextField4.getText().charAt(length-1);
       if(c<48 || c>57)  // equivalent to if(!(c=='0' || c=='1'|| c=='2'|| c=='3'|| c=='4'|| c=='5'|| c=='6'|| c=='7'|| c=='8'|| c=='9')) 
        jTextField4.setText(jTextField4.getText().substring(0, length-1));}
else if(length==1){
 char c =jTextField4.getText().charAt(0);
if(c<48 || c>57&&c!='-') jTextField4.setText(""); 
}
// TODO add your handling code here:
