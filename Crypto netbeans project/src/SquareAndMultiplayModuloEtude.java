...
public class SquareAndMultiplayModuloEtude extends javax.swing.JFrame {

    /**
     * Creates new form mainJfram
     */
    public SquareAndMultiplayModuloEtude() {
        initComponents();
...
/////////////////////////////////////////////////////////////////////////
    ...
    @Override
      ...
      /* publish(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n"
       ...
       fw.write(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n"
               + "Calcule finis: ( "+b.toString()+") ^ ( "+p.toString()+") mod ("+m.toString()+") = "+aPOWb.toString()+
               "\n<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n");
    
    
      fw.close();
      fw = new FileWriter("outNumber.txt");
      fw.write(aPOWb.toString());
      fw.close();
  ...
  protected void process(final List<String> chunks) { // hee type2
   jTextArea1.setText("");
    ...
    private void initComponents() {
        ...
        jLabel18.setText(" square and multiply (modulaire etude)");
...
File file = new File("outNumber.txt");
